...
const Searchcar = ({paramsquery,carbrands,provinces,year,carmodels}) => {

    const router = useRouter();
    const { params = [] } = router.query
    const [carbrand, setCarbrand] = useState(params[0]);
    const [carmodel, setCarmodel] = useState(params[1]);
    ...
    const Searchcarmodel = (e) => {
        ...
        router.push('/carforsale/' +(carbrand)+"/"+(e.target.value))
    }
    ...
    const Search = () => {
        ...
        if (pricemin && pricemax) {
            ...
            if (params[1]){
                router.push({
                    pathname: '/carforsale/'+params[0]+"/"+params[1]
                })
            }
            else {
                router.push({
                    pathname: '/carforsale/'+params[0]
                })
            }
        }
        if (params[1]){
    ...
    return (
        <div>
            <div className='d-flex justify-content-end'>
                <a onClick={ClearSearch}>ล้างการค้นหา</a>
            </div>
            <select className="form-select my-2" onChange={Searchcarbrand} value={carbrand}>
            ...
            <select className="form-select my-2" onChange={Searchcarmodel} value={carmodel}>
                <option>รุ่น</option>
                {carmodels.map((carmodel) => {
                    return (
                        <option key={carmodel.cm_id} value={carmodel.cm_name}>{carmodel.cm_name} ({carmodel.cm_count})</option>
                    )
                })
                }
            </select>
